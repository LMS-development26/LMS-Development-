...
import {
  LayoutDashboard, BookOpen, Users, ClipboardList, FileQuestion, Video,
  BarChart3, Menu, X, Bell, GraduationCap, LogOut, ChevronDown,
} from 'lucide-react';
...
const navItems: NavItem[] = [
  ...
  { to: '/instructor/enrollments', label: 'Enrollment Requests', icon: <ClipboardList className="h-5 w-5" /> },
  { to: '/instructor/students', label: 'Enrolled Students', icon: <Users className="h-5 w-5" /> },
];
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <aside className={classNames(
        ...
        <div className="flex h-16 items-center gap-2 border-b border-gray-200 px-6">
          ...
          <span className="text-lg font-bold text-gray-900">LMS Studio</span>
        </div>
        ...
        <div className="mt-auto p-4">
          <div className="rounded-lg bg-blue-50 p-4 text-center">
            <p className="text-xs text-blue-600 font-medium">Instructor Portal</p>
          </div>
        </div>
      </aside>
                ...
                <>
                  ...
                  <div className="absolute right-0 z-20 mt-2 w-80 rounded-xl border border-gray-200 bg-white shadow-lg">
                    ...
                    <div className="max-h-80 overflow-y-auto">
                      {notifications?.map((n) => (
                        <div key={n.id} className={classNames('border-b border-gray-50 px-4 py-3', !n.read && 'bg-blue-50/50')}>
            ...
            <div className="relative">
              ...
              {profileOpen && (
                <>
                  ...
                  <div className="absolute right-0 z-20 mt-2 w-56 rounded-xl border border-gray-200 bg-white shadow-lg">
                    <div className="border-b border-gray-100 px-4 py-3">
                      <p className="text-sm font-semibold text-gray-900">{user?.first_name} {user?.last_name}</p>
                      <p className="text-xs text-gray-500">{user?.email}</p>
                      <div className="mt-2"><StatusBadge status="INSTRUCTOR" /></div>
                    </div>
                    <div className="p-2">
                      <button
                        onClick={() => { switchUser('u-stu-1'); navigate('/student/courses'); }}
                        className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm text-gray-600 hover:bg-gray-50"
                      >
                        <GraduationCap className="h-4 w-4" /> Switch to Student
                      </button>
                      <button
                        onClick={() => { switchUser('u-admin-1'); navigate('/admin/dashboard'); }}
                        className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm text-gray-600 hover:bg-gray-50"
                      >
                        <LogOut className="h-4 w-4" /> Switch to Admin
                      </button>
                    </div>
                  </div>
                </>
